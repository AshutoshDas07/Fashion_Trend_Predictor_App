...
with open('Predicting_Trend\CSV\shirts.csv', mode='w') as file:
...
with open('Predicting_Trend\CSV\skirts.csv', mode='w') as file:
   writer = csv.writer(file, delimiter=',', lineterminator='\n')
   for index in range(pageIndex, lastPage + 1):
      page = http.request('GET', url + '/' + str(index))
      soup = BeautifulSoup(page.data)
...
with open('Predicting_Trend\CSV\pants.csv', mode='w') as file:
   writer = csv.writer(file, delimiter=',', lineterminator='\n')
   for index in range(pageIndex, lastPage + 1):
      page = http.request('GET', url + '/' + str(index))
      soup = BeautifulSoup(page.data)
...
with open('Predicting_Trend\CSV\dresses.csv', mode='w') as file:
   writer = csv.writer(file, delimiter=',', lineterminator='\n')
   for index in range(pageIndex, lastPage + 1):
      page = http.request('GET', url + '/' + str(index))
      soup = BeautifulSoup(page.data)
