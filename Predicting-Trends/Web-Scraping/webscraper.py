...
with open('CSV\shirts.csv', mode='w') as file:
...
with open('CSV\skirts.csv', mode='w') as file:
   writer = csv.writer(file, delimiter=',', lineterminator='\n')
   for index in range(pageIndex, lastPage + 1):
      page = http.request('GET', url + '/' + str(index))
      soup = BeautifulSoup(page.data)
...
with open('CSV\pants.csv', mode='w') as file:
   writer = csv.writer(file, delimiter=',', lineterminator='\n')
   for index in range(pageIndex, lastPage + 1):
      page = http.request('GET', url + '/' + str(index))
      soup = BeautifulSoup(page.data)
...
with open('CSV\dresses.csv', mode='w') as file:
   writer = csv.writer(file, delimiter=',', lineterminator='\n')
   for index in range(pageIndex, lastPage + 1):
      page = http.request('GET', url + '/' + str(index))
      soup = BeautifulSoup(page.data)
